...
title: "htsr-package"
author: 
  affiliation: HydroSciences Laboratory (IRD, CNRS, Univ. Montpellier), France
...
* `d_inventory(db.sqlite, stalist = NA, form.out = NA)` and `d_invent(fsq)`, makes the inventory of a htsr sqlite data base. The second is a Shiny interface of `d_inventory`
...
* `d_exp_hts(db.sqlite, sta, sen, rtime = FALSE, dstart, dend, rplot = FALSE)` and `d_exphts(fsq)`, extract hts files from a sqlite data base. The second is a shiny interface of `d_exp_hts`
...
* `p_plothts()`, plots hts files. Shiny interface of `p_line` & `p_bar`
